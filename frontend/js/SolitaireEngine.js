...
/***/ function(module, exports, __webpack_require__) {
	...
	var _winCheck = __webpack_require__(65);
	...
	exports.version = (9091495514).toString().split(9).slice(1).map(function (e) {
		return parseInt(e, 8);
	}).join('.');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _specialStep = __webpack_require__(63);
	...
	var checkTips = function checkTips(e) {
		...
		if (_showTips) {
			...
			for (var i in _tips) {
				...
				// )                                                       ||
				_field2.default.tipsParams.excludeHomeGroups && _homeGroups && _homeGroups.length && _homeGroups.indexOf(_tips[i].from.deck.parent) >= 0) {
					// ?#$%&!
				} else {
				...
				for (; count; count -= 1) {
					...
					for (var i = 0; i < this.cards.length; i += 1) {
						if (this.cards[i].id == _pop.id) {
							this.cards = [].concat(_toConsumableArray(this.cards.slice(0, i)), _toConsumableArray(this.cards.slice(i + 1)));
						}
					}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _rollerAction = __webpack_require__(33);
			...
			value: function run(deck, data) {
				...
				if (cardsCount > 0) {
					...
					if (cardsCount == 0) {
						...
						if (deck.data.rollerActionData && deck.data.rollerActionData.cardsCount && deck.data.rollerActionData.cardsCount == hiddenCardsCount) {
	
							_event2.default.dispatch('rewindHistory', function (data) {
	
								for (var _i2 = 0; _i2 < deck.data.rollerActionData.stepsCount; _i2 += 1) {
									data.undo();
								}
							});
						} else {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _undo = function _undo(data) {
		...
		// UNFLIP
		if (data.unflip) {
			var deck = _common2.default.getElementByName(data.unflip.deckName, 'deck');
			var card = deck.getCardByIndex(data.unflip.cardIndex);
			if (card) {
		...
		// HIDE
		if (data.hide) {
			var _deck = _common2.default.getElementByName(data.hide.deckName, 'deck');
			if (_deck.cards[data.hide.cardIndex].name == data.hide.cardName) {
				_deck.cards[data.hide.cardIndex].visible = true;
				_deck.Redraw();
			}
		}
	...
	var _redo = function _redo(data) {
		...
		// UNFLIP
		if (data.unflip) {
			var deck = _common2.default.getElementByName(data.unflip.deckName);
			var card = deck.getCardByIndex(data.unflip.cardIndex | 0);
			if (card) {
				card.flip = false;
				// event.dispatch('redrawDeckFlip', deck);
				deck.Redraw();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _winCheckRules2 = __webpack_require__(66);
	...
	var winCheck = function winCheck(params) {
		...
		for (var ruleName in _winCheckRules) {
			...
			var _ruleName = ruleName.length == 1 && ruleName | 0 == 0 ? _winCheckRules[ruleName] : ruleName;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _elClass = __webpack_require__(69);
	
	var _elClass2 = _interopRequireDefault(_elClass);
	
	var _allElClass = __webpack_require__(70);
...
/***/ function(module, exports, __webpack_require__) {
	...
	_event2.default.listen('toggleMarkCard', function (data) {
		...
		var el = _share2.default.get('domElement:' + data.id);
